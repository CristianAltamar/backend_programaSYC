...
export const getOrderClient = async (filters = {}, limit = 10, page = 1) => {
    ...
    const [orders]= await pool.query(
        `SELECT o.purchase_order_id as id, o.date, c.name as client, o.quote_id, o.delivery, o.attached_order
        FROM purchase_orders o
        JOIN clients c ON o.client_id = c.UniqueID
    ...
    const [[{total}]] = await pool.query(
        ...
        FROM purchase_orders o 
        INNER JOIN clients c ON o.client_id = c.UniqueID 
        ${whereClause}`, 
        values
    );
...
export const getOrderClientByID = async (id) => {
    ...
    try {
        ...
        const [order] = await connection.query(
            `SELECT o.purchase_order_id as id, o.date, c.nit as clientID, c.name as client, o.quote_id, o.delivery, o.attached_order
            FROM purchase_orders o
            JOIN clients c ON o.client_id = c.UniqueID
...
export const createOrderClient = async ({id, date, clientID, quoteID, delivery, attached, products}) => {
    ...
    try {
        ...
        const [result] = await connection.query(
            'INSERT INTO purchase_orders (purchase_order_id, date, client_id, quote_id, delivery, attached_order) VALUES (?, ?, ?, ?, ?, ?)',
            [id, date, clientID, quoteID, delivery, attached]
        );
...
export const updateOrderClient = async (id, generalDates, products) => {
    ...
    try {
        ...
        await connection.query(
            'UPDATE purchase_orders SET date = ?, client_id = ?, quote_id = ?, delivery = ?, attached_order = ? WHERE purchase_order_id = ?',
            [generalDates.date, generalDates.clientID, generalDates.quoteID, generalDates.delivery, generalDates.attached, id]
        );
...
export const deleteOrderClient = async (ID) => {
